    ...
    <div class="card" style="margin-bottom:16px;overflow:hidden;">
        ...
        <div style="padding:24px;display:grid;grid-template-columns:1fr 1fr;gap:18px;">
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('name') input-error @enderror" name="name" value="{{ old('name', $user->getRawOriginal('name')) }}" placeholder="{{ $user->name }}">
                @error('name')<div class="field-error">{{ $message }}</div>@enderror
            </div>
            <div class="form-group">
                <label class="form-label">Email <span style="color:#f43f5e;">*</span></label>
                <input type="email" class="form-control @error('email') input-error @enderror" name="email" value="{{ old('email', $user->email) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control form-select @error('staff_id') input-error @enderror" name="staff_id">
                    ...
                    @foreach($availableStaff as $s)
                        <option value="{{ $s->id }}" {{ old('staff_id', $user->staff_id) == $s->id ? 'selected' : '' }}>{{ $s->full_name }} @if($s->specialite) — {{ $s->specialite }} @endif</option>
                    @endforeach
                </select>
...
{{-- Reset password — separate form, admin sets it directly, no current-password check --}}
<div class="card" style="overflow:hidden;">
    <div style="padding:20px 24px 18px;background:linear-gradient(135deg,rgba(245,158,11,.04),rgba(255,255,255,0));border-bottom:1px solid rgba(245,158,11,.1);display:flex;align-items:center;gap:14px;">
        <div style="width:40px;height:40px;border-radius:12px;background:linear-gradient(135deg,#fbbf24,#f59e0b);display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 4px 12px rgba(245,158,11,.3);">
            <svg width="18" height="18" fill="none" stroke="#fff" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z"/></svg>
        </div>
        <div>
            <h3 style="font-size:15px;font-weight:700;color:var(--teal-800);margin:0;line-height:1.2;">Réinitialiser le mot de passe</h3>
            <span style="font-size:12px;color:var(--muted);font-weight:500;">Aucun accès en libre-service n'est prévu — c'est ici que l'administrateur définit un nouveau mot de passe pour ce compte</span>
        </div>
    </div>
