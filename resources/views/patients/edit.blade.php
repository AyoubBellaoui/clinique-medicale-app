...
@section('page-subtitle', 'Mettre à jour le dossier de {{ $p->prenom }} {{ $p->nom }}')
...
<!------------------------------------------------------- Patient Form ------------------------------------------------->
<form action="{{ route('patients.update', $p->id) }}" method="POST" id="patientForm">
@csrf
@method('PUT')
                ...
                <div style="padding:24px;display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    ...
                    <div style="grid-column:1/-1;padding:16px;border-radius:12px;background:rgba(52,168,140,.04);border:1.5px dashed rgba(52,168,140,.2);">
                        ...
                        <div style="display:flex;gap:8px;align-items:center;">
                            ...
                            @php $isSelected = old('color', $p->color) === $cname; @endphp
                            <label style="cursor:pointer;">
                                <input type="radio" name="color" value="{{ $cname }}" {{ $isSelected ? 'checked' : '' }} style="display:none;" onchange="updatePreviewColor('{{ $cname }}','{{ $chex }}')">
                                <div class="color-dot {{ $isSelected ? 'selected' : '' }}" data-color="{{ $cname }}" style="width:32px;height:32px;border-radius:50%;background:{{ $chex }};cursor:pointer;border:3px solid {{ $isSelected ? 'white' : 'transparent' }};box-shadow:{{ $isSelected ? '0 0 0 2px '.$chex.', 0 3px 8px rgba(0,0,0,.15)' : '0 2px 6px rgba(0,0,0,.1)' }};transition:all .2s;display:flex;align-items:center;justify-content:center;">
                                    @if($isSelected)
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div style="position:relative;">
                            ...
                            <input type="text" class="form-control pf-input @error('prenom') input-error @enderror" name="prenom" id="prenom" placeholder="Prénom" value="{{ old('prenom', $p->prenom) }}" oninput="updatePreview()" style="padding-left:36px;">
                        </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div style="position:relative;">
                            <svg style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--muted);pointer-events:none;" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z"/></svg>
                            <input type="text" class="form-control pf-input @error('nom') input-error @enderror" name="nom" id="nom" placeholder="Nom de famille" value="{{ old('nom', $p->nom) }}" oninput="updatePreview()" style="padding-left:36px;">
                        </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div style="position:relative;">
                            ...
                            <input type="text" class="form-control text-mono pf-input @error('cin') input-error @enderror" name="cin" id="cin_field" placeholder="Ex: A123456" value="{{ old('cin', $p->cin) }}" oninput="updatePreview()" style="padding-left:36px;letter-spacing:.05em;">
                        </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div style="position:relative;">
                            ...
                            <input datepicker datepicker-autohide datepicker-format="yyyy-mm-dd" type="text" class="form-control" name="date_naissance" id="dob_field" onchange="updatePreview()" placeholder="aaaa-mm-jj" autocomplete="off" value="{{ old('date_naissance', $p->date_naissance ? \Carbon\Carbon::parse($p->date_naissance)->format('Y-m-d') : '') }}" style="padding-left:36px;">
                        </div>
                    </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <input type="hidden" name="genre" id="gender_input" value="{{ old('genre', $p->genre) }}">
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                            <button type="button" class="gender-btn {{ old('genre', $p->genre) === 'M' ? 'active' : '' }}" id="btn-M" onclick="selectGender('M')" style="padding:10px;border-radius:10px;border:1.5px solid rgba(52,168,140,.2);background:#fff;color:var(--muted);font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:7px;transition:all .2s;">
                            ...
                            <button type="button" class="gender-btn {{ old('genre', $p->genre) === 'F' ? 'active' : '' }}" id="btn-F" onclick="selectGender('F')" style="padding:10px;border-radius:10px;border:1.5px solid rgba(52,168,140,.2);background:#fff;color:var(--muted);font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:7px;transition:all .2s;">
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <input type="hidden" name="groupe_sanguin" id="blood_input" value="{{ old('groupe_sanguin', $p->groupe_sanguin) }}">
                        <div style="display:flex;flex-wrap:wrap;gap:6px;">
                            @foreach(['A+','A-','B+','B-','AB+','AB-','O+','O-'] as $bt)
                            <button type="button" class="blood-btn {{ old('groupe_sanguin', $p->groupe_sanguin) === $bt ? 'active' : '' }}" data-bt="{{ $bt }}" onclick="selectBlood('{{ $bt }}')" style="padding:7px 12px;border-radius:8px;border:1.5px solid rgba(52,168,140,.2);background:#fff;color:var(--muted);font-size:12.5px;font-weight:700;cursor:pointer;transition:all .2s;font-variant-numeric:tabular-nums;">
                                {{ $bt }}
                            </button>
                            @endforeach
                        </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div style="position:relative;">
                            ...
                            <input type="tel" class="form-control pf-input @error('telephone') input-error @enderror" name="telephone" id="phone_field" placeholder="+212 6xx xxx xxx" value="{{ old('telephone', $p->telephone) }}" oninput="updatePreview()" style="padding-left:36px;">
                        </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div style="position:relative;">
                            ...
                            <input type="email" class="form-control @error('email') input-error @enderror" name="email" placeholder="patient@email.com" value="{{ old('email', $p->email) }}" style="padding-left:36px;">
                        </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;grid-column:1/-1;">
                        ...
                        <div style="position:relative;">
                            ...
                            <input type="text" class="form-control" name="adresse" placeholder="Adresse complète" value="{{ old('adresse', $p->adresse) }}" style="padding-left:36px;">
                        </div>
                    </div>
            ...
            <div class="card" style="margin-bottom:16px;overflow:hidden;">
                ...
                <div style="padding:24px;display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    ...
                    <div class="form-group" style="margin-bottom:0;grid-column:1/-1;">
                        ...
                        <textarea class="form-control" name="antecedents" rows="4" placeholder="Maladies chroniques, chirurgies antérieures, hospitalisations, traitements en cours…" style="resize:vertical;line-height:1.6;">{{ old('antecedents', $p->antecedents) }}</textarea>
                    </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div style="position:relative;">
                            ...
                            <select class="form-control form-select" name="medecin_id" id="doctor_field" onchange="updatePreview()" style="padding-left:36px;">
                                ...
                                <option value="{{ $doctor->id }}" {{ old('medecin_id', $p->medecin_id) == $doctor->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <input type="hidden" name="statut_dossier" id="status_input" value="{{ old('statut_dossier', $p->statut_dossier ?? 'actif') }}">
                        @php $currentStatus = old('statut_dossier', $p->statut_dossier ?? 'actif'); @endphp
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                            <button type="button" id="btn-actif" onclick="selectStatus('actif')" style="padding:10px;border-radius:10px;border:1.5px solid {{ $currentStatus === 'actif' ? 'rgba(16,185,129,.4)' : 'rgba(52,168,140,.2)' }};background:{{ $currentStatus === 'actif' ? 'rgba(16,185,129,.08)' : '#fff' }};color:{{ $currentStatus === 'actif' ? '#059669' : 'var(--muted)' }};font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;transition:all .2s;">
                                <span style="width:8px;height:8px;border-radius:50%;background:#10b981;"></span> Actif
                            </button>
                            <button type="button" id="btn-inactif" onclick="selectStatus('inactif')" style="padding:10px;border-radius:10px;border:1.5px solid {{ $currentStatus === 'inactif' ? 'rgba(100,116,139,.3)' : 'rgba(52,168,140,.2)' }};background:{{ $currentStatus === 'inactif' ? 'rgba(100,116,139,.08)' : '#fff' }};color:{{ $currentStatus === 'inactif' ? '#475569' : 'var(--muted)' }};font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;transition:all .2s;">
            ...
            <div class="card" style="margin-bottom:16px;overflow:hidden;">
                ...
                <div style="padding:24px;display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    ...
                    <div class="form-group" style="margin-bottom:0;grid-column:1/-1;">
                        ...
                        <input type="hidden" name="assurance_type" id="insurance_input" value="{{ old('assurance_type', $p->assurance_type) }}">
                        <div style="display:flex;flex-wrap:wrap;gap:8px;">
                            @foreach(['Aucune','CNSS','CNOPS','FAR','Privée','Autre'] as $ins)
                            <button type="button" class="ins-btn {{ old('assurance_type', $p->assurance_type) === $ins ? 'active' : '' }}" data-ins="{{ $ins }}" onclick="selectInsurance('{{ $ins }}')" style="padding:8px 16px;border-radius:10px;border:1.5px solid rgba(59,130,246,.2);background:#fff;color:var(--muted);font-size:13px;font-weight:600;cursor:pointer;transition:all .2s;">
                                {{ $ins }}
                            </button>
                            @endforeach
                        </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;grid-column:1/-1;">
                        ...
                        <div style="position:relative;">
                            ...
                            <input type="text" class="form-control" name="assurance_numero" placeholder="Numéro d'affiliation" value="{{ old('assurance_numero', $p->assurance_numero) }}" style="padding-left:36px;">
                        </div>
                    </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div style="position:relative;">
                            ...
                            <input type="text" class="form-control" name="contact_urgence_nom" placeholder="Nom complet" value="{{ old('contact_urgence_nom', $p->contact_urgence_nom) }}" style="padding-left:36px;">
                        </div>
                    </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div style="position:relative;">
                            <svg style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--muted);pointer-events:none;" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z"/></svg>
                            <input type="tel" class="form-control" name="contact_urgence_tel" placeholder="+212 6xx xxx xxx" value="{{ old('contact_urgence_tel', $p->contact_urgence_tel) }}" style="padding-left:36px;">
                        </div>
                    </div>
                    ...
                    <div class="form-group" style="margin-bottom:0;grid-column:1/-1;">
                        ...
                        <input type="hidden" name="lien_urgence" id="relation_input" value="{{ old('lien_urgence', $p->lien_urgence) }}">
                        <div style="display:flex;flex-wrap:wrap;gap:8px;">
                            @foreach(['Conjoint(e)','Parent','Enfant','Frère / Sœur','Ami(e)','Autre'] as $rel)
                            <button type="button" class="rel-btn {{ old('lien_urgence', $p->lien_urgence) === $rel ? 'active' : '' }}" data-rel="{{ $rel }}" onclick="selectRelation('{{ $rel }}')" style="padding:7px 14px;border-radius:8px;border:1.5px solid rgba(52,168,140,.2);background:#fff;color:var(--muted);font-size:12.5px;font-weight:600;cursor:pointer;transition:all .2s;">
                                {{ $rel }}
                            </button>
                            @endforeach
                        </div>
    ...
    <div style="position:sticky;top:calc(var(--header-h) + 20px);">
        ...
        <div class="card" style="overflow:hidden;margin-bottom:14px;">
            ...
            <div style="padding:10px 20px 20px;">
                <div id="preview-name" style="font-size:16px;font-weight:800;color:var(--teal-800);margin-bottom:2px;line-height:1.2;">{{ $p->prenom }} {{ $p->nom }}</div>
                <div id="preview-cin" style="font-size:12px;color:var(--muted);font-family:monospace;margin-bottom:12px;">CIN: {{ strtoupper($p->cin) }}</div>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:14px;">
                    <div style="text-align:center;padding:8px 6px;border-radius:10px;background:var(--teal-50);border:1px solid rgba(52,168,140,.12);">
                        <div id="preview-age" style="font-size:16px;font-weight:800;color:var(--teal-700);">
                            {{ $p->date_naissance ? floor((now()->diffInDays(\Carbon\Carbon::parse($p->date_naissance))) / 365.25) . ' ans' : '—' }}
                        </div>
                        <div style="font-size:10px;color:var(--muted);font-weight:600;">Âge</div>
                    </div>
                    <div style="text-align:center;padding:8px 6px;border-radius:10px;background:var(--teal-50);border:1px solid rgba(52,168,140,.12);">
                        <div id="preview-blood" style="font-size:16px;font-weight:800;color:var(--teal-700);">{{ $p->groupe_sanguin ?? '—' }}</div>
                        <div style="font-size:10px;color:var(--muted);font-weight:600;">Groupe</div>
                    </div>
                    <div style="text-align:center;padding:8px 6px;border-radius:10px;background:var(--teal-50);border:1px solid rgba(52,168,140,.12);">
                        <div id="preview-gender-icon" style="font-size:15px;font-weight:800;color:var(--teal-700);">
                            {{ $p->genre === 'M' ? '♂' : ($p->genre === 'F' ? '♀' : '—') }}
                        </div>
                ...
                <div style="display:flex;flex-direction:column;gap:7px;">
                    <div style="display:flex;align-items:center;gap:8px;font-size:12.5px;">
                        ...
                        <span id="preview-phone" style="color:{{ $p->telephone ? 'var(--teal-700)' : 'var(--muted)' }};">{{ $p->telephone ?? 'Téléphone non renseigné' }}</span>
                    </div>
                    <div style="display:flex;align-items:center;gap:8px;font-size:12.5px;">
                    ...
                    <div style="display:flex;align-items:center;gap:8px;font-size:12.5px;margin-top:4px;">
                        <span id="preview-status-badge" class="badge {{ ($p->statut_dossier ?? 'actif') === 'actif' ? 'badge-green' : 'badge-gray' }}">
                            {{ ($p->statut_dossier ?? 'actif') === 'actif' ? 'Actif' : 'Inactif' }}
                        </span>
                    </div>
                </div>
            </div>
...
.gender-btn, .blood-btn, .ins-btn, .rel-btn { font-family: 'Plus Jakarta Sans', sans-serif; transition: all .18s; }
...
/* ── Init preview with existing color ── */
(function () {
    const selected = document.querySelector('input[name="color"]:checked');
    if (selected) updatePreviewColor(selected.value, colorHex[selected.value]);
    updatePreview();
})();
</script>
@endpush
